...
from OCC.Core.RWMesh import RWMesh_CoordinateSystem_negZfwd_posYup, RWMesh_CoordinateSystem_posYfwd_posZup
...
def check_svgwrite_installed():
    if not HAVE_SVGWRITE:
        raise IOError("svg exporter not available because the svgwrite package is not installed. use $pip install svgwrite'")
...
def write_step_file(a_shape, filename, application_protocol="AP203"):
    ...
    if application_protocol not in ["AP203", "AP214IS", "AP242DIS"]:
        raise AssertionError(f"application_protocol must be either AP203 or AP214IS. You passed {application_protocol}.")
...
def write_ply_file(a_shape, ply_filename):
    ...
    a_file_info.Add(TCollection_AsciiString("Authors"), TCollection_AsciiString("pythonocc"))
