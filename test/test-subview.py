...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.viewer.CreateView()
        subview = V3d_View(self.viewer)
        self.view.AddSubview(subview)
        subview.SetImmediateUpdate(True)
        subview.SetWindow(
            self.view, Graphic3d_Vec2d(0.5, 1.0), Aspect_TOTP_LEFT_UPPER, Graphic3d_Vec2d(0.5, 0.0)
        )

        # main_widget = QHBoxLayout()
        # main_widget.addWidget(self.canvas)
...
def test(gui: MainWindow):
    ...
    ========================================================================================
    """
    # box_shape = readStep("./robot-mods/J3.STEP")
    shape, color = readStepWithColor(r"D:\ICO\PipeBending\mods\pipbendmachine\模具底座 Y.STEP")
    # old_ais = AIS_Shape(box_shape)
    # ais_box = AIS_Shape(box_shape)
    # a_trsf = gp_Trsf()
    # a_trsf.SetTranslation(gp_Vec(3, 2, 6))
    # ais_box.SetLocalTransformation(a_trsf)
    # old_ais_sha = AIS_Shape_Tri(old_ais)
    # texture_shape = AIS_Shape_Tri(ais_box)
    ais_shape = AIS_Shape(shape)
    ais_shape.SetColor(color)
    # gui.display(ais_shape, True)
    gui.v3d.DisplayVector(gp_Vec(100, 0, 0), gp_Pnt(0, 0, 0))
    gui.v3d.DisplayVector(gp_Vec(10, 0, 0), gp_Pnt(0, 0, 0))
    gui.v3d.FitAll()
    # gui.display(ais_shape, True)
