import math
import time
from pprint import pprint

from context import *

LINEAR_TOLERANCE_LIST = [
    1e-6,
    1e-4,
    1e-2,
    0.1,
]


class Edge:
    def __init__(self, edge: TopoDS_Edge):
        self.topo_edge: TopoDS_Edge = edge
        self._get_ends()

    # end alternate constructor
    def _get_ends(self):
        aBAC = BRepAdaptor_Curve(self.topo_edge)
        self.start = aBAC.Value(aBAC.FirstParameter())
        self.end = aBAC.Value(aBAC.LastParameter())
        if self.start.Distance(gp_Pnt()) > self.end.Distance(gp_Pnt()):
            self.start, self.end = self.end, self.start

    # end def
    def __repr__(self) -> str:
        return f"{print_gp_Pnt(self.start,1,True,False)}->{print_gp_Pnt(self.end,1,True,False)}"

    # end def


# end def
def getEdges(shape: TopoDS_Shape):
    result: list[Edge] = []
    explorer = TopExp_Explorer(shape, TopAbs_EDGE)
    while explorer.More():
        e = explorer.Current()
        edge = Edge(topods.Edge(e))
        result.append(edge)
        explorer.Next()
    # end while
    return result


# end def
def make_piece(edge: Edge):
    p1 = edge.start
    p2 = edge.end
    p3 = gp_Pnt(p2.X(), p2.Y(), 0)
    p4 = gp_Pnt(p1.X(), p1.Y(), 0)
    l1 = edge.topo_edge
    l2 = BRepBuilderAPI_MakeEdge(p2, p3).Edge()
    l3 = BRepBuilderAPI_MakeEdge(p3, p4).Edge()
    l4 = BRepBuilderAPI_MakeEdge(p4, p1).Edge()
    wire_maker = BRepBuilderAPI_MakeWire()
    wire_maker.Add(l1)
    wire_maker.Add(l2)
    wire_maker.Add(l3)
    wire_maker.Add(l4)
    face_maker = BRepBuilderAPI_MakeFace(wire_maker.Wire(), True)
    return face_maker.Face()


# end def


def add_edge(edge_dict: dict, edge_list: list[Edge]):
    global GUI
    for e in edge_list:
        for tol in LINEAR_TOLERANCE_LIST:
            if (
                e.start.Distance(edge_dict["end"]) < tol
                and e.end.Distance(edge_dict["origin"]) > edge_dict["end_distance"]
            ):
                edge_dict["end"] = e.end
                edge_dict["end_distance"] = e.end.Distance(edge_dict["origin"])
                edge_dict["edge"].append(e)
                ais_piece = AIS_Shape(e.topo_edge)
                ais_piece.SetWidth(5)
                ais_piece.SetColor(getColor(255, 0, 0))
                # GUI.display(ais_piece, True)
                # time.sleep(0.2)
                GUI.v3d.FitAll()
                return False
            elif (
                e.end.Distance(edge_dict["start"]) < tol
                and e.start.Distance(edge_dict["origin"]) > edge_dict["start_distance"]
            ):
                edge_dict["start"] = e.start
                edge_dict["start_distance"] = e.start.Distance(edge_dict["origin"])
                edge_dict["edge"].insert(0, e)
                ais_piece = AIS_Shape(e.topo_edge)
                ais_piece.SetWidth(5)
                ais_piece.SetColor(getColor(0, 255, 0))
                # GUI.display(ais_piece, True)
                # time.sleep(0.2)
                GUI.v3d.FitAll()
                return False
    #     test.append(
    #         f"start {edge_dict['start'].X():1f}, end {edge_dict['end'].X():1f}, ps {e.start.X():1f}, pe {e.end.X():1f}"
    #     )
    # pprint(test)
    return True


# end def


# # end def


def test(gui: MainWindow):
    """
    ========================================================================================
    TEST
    ========================================================================================
    """
    gui.canvas.display_graduated_trihedron()
    gui.canvas.viewer3d.View_Right()

    input_point = gp_Pnt(0, 0, 0)
    input_dir = gp_Dir(1, 0, 0)

    # autocorrect:false
    #! 读取工件
    # input_workpiece_shape = read_step(r"D:\Projects\FANUC激光切割机器人\简易夹具\无厚度工件.stp")
    input_workpiece_shape = read_step(r".\laser-test1-Small-01.stp")
    print(input_workpiece_shape.ShapeType())
    explorer = TopExp_Explorer(input_workpiece_shape, TopAbs_SHELL)
    while explorer.More():
        e = explorer.Current()
        print(f"{TopAbs_ShapeEnum(e.ShapeType()).name}")
        gui.display(AIS_Shape(e), True)
        explorer.Next()
    # end while
    # autocorrect:true

    ais_box_shape = AIS_Shape(input_workpiece_shape)
    ais_box_shape.SetTransparency(0.9)
    ais_box_shape.SetColor(COLOR.OCC_GRAY)
    gui.display(ais_box_shape, True)

    #! 创建截面并切割
    cross_plane = BRepBuilderAPI_MakeFace(gp_Pln(input_point, input_dir)).Face()
    cross_shape = BRepAlgoAPI_Section(input_workpiece_shape, cross_plane).Shape()

    ais_cross = AIS_Shape(cross_shape)
    gui.display(ais_cross, True)

    #! 获取截面线
    edges: list[Edge] = getEdges(cross_shape)
    return

    # # * 移除竖直的线
    # new_edge: list[Edge] = []
    # z_dir = gp_Vec(0, 0, 1)
    # for e in edges:
    #     if gp_Vec(e.start, e.end).IsParallel(z_dir, 0.1):
    #         continue
    #     else:
    #         new_edge.append(e)
    # # end for

    # total_Z = 0
    # for e in edges:
    #     total_Z += e.start.Z()
    #     total_Z += e.end.Z()
    # # end for
    # the_Z = total_Z / len(edges) / 2

    new_edge: list[Edge] = sorted(edges, key=lambda x: x.start.Distance(gp_Pnt()))

    max_point = new_edge[0]

    for e in new_edge:
        if e.start.Distance(max_point.start) > e.end.Distance(max_point.start):
            e.start, e.end = e.end, e.start
        # end if
    # end for
    new_edge: list[Edge] = sorted(edges, key=lambda x: x.start.Distance(max_point.start))

    # ais_vertex = AIS_Shape(BRepBuilderAPI_MakeVertex(new_edge[0].start).Shape())
    # ais_vertex.SetWidth(10)
    # gui.display(ais_vertex, True)

    wire_maker = BRepBuilderAPI_MakeWire()
    container = TopTools_ListOfShape()
    for e in new_edge:
        container.Append(e.topo_edge)
    # end for
    wire_maker.Add(container)
    wire_explorer = BRepTools_WireExplorer(wire_maker.Wire())
    new_edge = []
    while wire_explorer.More():
        a_edge = Edge(wire_explorer.Current())
        current_point = BRep_Tool.Pnt(wire_explorer.CurrentVertex())
        if a_edge.start.Distance(current_point) > a_edge.end.Distance(current_point):
            a_edge.start, a_edge.end = a_edge.end, a_edge.start

        new_edge.append(a_edge)
        wire_explorer.Next()
    # end while

    # region #! debug display

    # for e in new_edge:
    #     color = next(color_list)
    #     ais_edge = AIS_Shape(e.topo_edge)
    #     ais_edge.SetColor(color)
    #     ais_edge.SetWidth(2)
    #     gui.display(ais_edge, False)

    #     start_vertex = BRepBuilderAPI_MakeVertex(e.start).Shape()
    #     ais_start = AIS_Shape(start_vertex)
    #     ais_start.SetColor(color)
    #     ais_start.SetWidth(5)
    #     gui.display(ais_start, True)
    # # end for

    # endregion

    first_dict = {
        "start": gp_Pnt(),
        "start_distance": 0.0,
        "end": gp_Pnt(),
        "end_distance": 0.0,
        "edge": [],
        "origin": gp_Pnt(),
    }
    # for i in new_edge:
    #     print("start: " + printPnt(i.start, 1, True) + "  " + "end: " + printPnt(i.end, 1, True))
    #     gui.display(AIS_Shape(BRepBuilderAPI_MakeWire(i.topo_edge).Shape()), False)
    # # end for
    #! 找到其中一条边
    finish = False
    while not finish:
        if not first_dict["edge"]:
            first_dict["start"] = new_edge[0].start
            first_dict["origin"] = new_edge[0].start
            first_dict["end"] = new_edge[0].end
            first_dict["edge"].append(new_edge[0])

            ais_vertex = AIS_Shape(BRepBuilderAPI_MakeVertex(new_edge[0].start).Shape())
            ais_vertex.SetWidth(10)
            gui.display(ais_vertex, True)

        # end for
        finish = add_edge(first_dict, new_edge)

    # end for

    #! 划分另一条边
    second_list: list[Edge] = []
    for e in new_edge:
        if e not in first_dict["edge"]:
            second_list.append(e)
        # end if
    # end for

    # * 显示
    for e in first_dict["edge"]:
        ais_edge = AIS_Shape(e.topo_edge)
        ais_edge.SetColor(getColor(255, 0, 0))
        ais_edge.SetWidth(5)
        print(
            "start: "
            + print_gp_Pnt(e.start, 1, True)
            + "  "
            + "end: "
            + print_gp_Pnt(e.end, 1, True)
        )
        gui.display(ais_edge, False)
        # ais_piece = AIS_Shape(make_piece(e))
        # ais_piece.SetColor(getColor(255, 0, 0))
        # ais_piece.SetTransparency(0.5)
        # # print("start: " + printPnt(e.start, 1, True) + "  " + "end: " + printPnt(e.end, 1, True))
        # gui.display(ais_piece, False)
    # end for

    # for e in second_list:
    #     ais_sed = AIS_Shape(e.topo_edge)
    #     ais_sed.SetWidth(5)
    #     ais_sed.SetTransparency(0.9)
    #     ais_sed.SetColor(getColor(0, 255, 0))
    #     gui.display(ais_sed, False)
    # # end for

    gui.view.Update()
    gui.v3d.FitAll()


# end def
def mainGUI():
    app = QApplication().instance()
    if not app:
        app = QApplication(sys.argv)
    gui = MainWindow()
    gui.show()
    gui.canvas.InitDriver()
    gui.canvas.qApp = app
    global GUI
    GUI = gui
    test(gui)
    # 窗口置顶
    gui.raise_()
    app.exec()


GUI: MainWindow = None
# end def
if __name__ == "__main__":
    mainGUI()
# end main
