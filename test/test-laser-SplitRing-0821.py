...
def get_edges(gui: MainWindow, cross: BRepAlgoAPI_Splitter):
    #! 获取截面线
    edges: list[Edge] = []
    edges_list = cross.SectionEdges()
    for i in range(edges_list.Size()):
        e = edges_list.Value(i + 1)
        edge = Edge(topods.Edge(e))
        edges.append(edge)
    # end for
    if not edges:
...
def test(gui: MainWindow):
    ...
    for xp in x_point_list:
        ...
        # ais_cross_plane = AIS_Shape(cross_plane)
        # gui.display(ais_cross_plane, False)
        face_explorer = TopExp_Explorer(input_workpiece_shape, TopAbs_FACE)
        true_num = 0
        false_num = 0
        int_tool = IntTools_FaceFace()
        count_shape_types(input_workpiece_shape)
        while face_explorer.More():
            a_face = topods.Face(face_explorer.Current())
            section = BRepAlgoAPI_Section(a_face, cross_plane, False)
            section.Approximation(True)
            section.ComputePCurveOn1(True)
            section.SetFuzzyValue(0.1)
            section.Build()

            if section.IsDone():
                # split = BRepFeat_SplitShape(a_face)
                # explorer = TopExp_Explorer(section.Shape(), TopAbs_EDGE)
                # while explorer.More():
                #     anEdge = topods.Edge(explorer.Current())
                #     aFace = TopoDS_Shape()
                #     if section.HasAncestorFaceOn1(anEdge, aFace):
                #         split.Add(anEdge, topods.Face(aFace))
                #     explorer.Next()
                # # end while

                true_num += 1
                # print(f"section is done: {true_num}")
                ais_cross = AIS_Shape(a_face)
                gui.display(ais_cross, False)
            else:
                false_num += 1
                print(f"section is not done: {false_num}")
            # end if
            face_explorer.Next()
        # end while
        break
...
#             explorer.Next()
#         # end while
#         ais_cross = AIS_Shape(split.Shape())
#         gui.display(ais_cross, False)
#     else:
#         num += 1
#         print(f"section is not done{num}")
#     # end if
#     face_explorer.Next()
