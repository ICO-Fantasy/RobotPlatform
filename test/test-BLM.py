...
def test(gui: MainWindow):
    ...
    # 定义文件夹路径
    folder_path = Path(r"D:\Projects\中车弯管\moldes\BLM-new")

    # 遍历文件夹及其所有子文件夹中的 STEP 文件
    step_files = list(folder_path.glob("**/*.STEP")) + list(folder_path.glob("**/*.STEP"))
    # for stp in step_files:
    #     box_shape = readStep(str(stp))
    #     ais_box = AIS_Shape(box_shape)
    #     gui.display(ais_box, False)
    box_shape = read_step(
        r"D:\Projects\中车弯管\moldes\BLM-new\Spostamento testa-semplificato.STEP"
    )
    ais_box = AIS_Shape(box_shape)
    gui.display(ais_box, False)
    gui.v3d.Context.UpdateCurrentViewer()
    gui.v3d.FitAll()
