...
from OCC.Core.Prs3d import Prs3d_DatumAspect, Prs3d_DatumParts
...
from OCC.Core.V3d import V3d_View, V3d_ZBUFFER
...
from pyOCC import LocalCONFIG, createTrihedron, getColor
...
class qtViewer3dWidget(qtBaseViewerWidget):
    ...
    def InitDriver(self):
        ...
        self.viewer3d.Create(
            window_handle=int(self.winId()), parent=self, create_default_lights=False
        )
        # 设置显示实体
        ...
        self.viewer3d.View.SetBgGradientColors(
            getColor(self._bg_gradient_color[0]),
            getColor(self._bg_gradient_color[1]),
        ...
        if True:
            # 开启光追效果
            self.viewer3d.SetRaytracingMode(depth=3)
        if False:
            # 开启光栅化效果
    ...
    def _calculate_draw_box(self, event: QtGui.QMouseEvent, tolerance=2):
        ...
        # todo 绘制选择框
        self.drawRubberBand(
            self.drag_start_position_x, self.drag_start_position_y, point.x(), point.y()
        )
