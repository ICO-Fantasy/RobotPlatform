...
from . import LocalCONFIG, getColor
...
def createTrihedron(input: Union[AIS_Shape, TopLoc_Location, gp_Trsf, None] = None, arrow_length: int = 200) -> AIS_Trihedron:
    # 根据输入类型进行处理
    match input:
        case AIS_Shape():
            a_trsf = input.Shape().Location().Transformation()
    ...
    a_trihedron.Attributes().DatumAspect().LineAspect(X_AXIS_DIRECTION).SetWidth(2.5)
    a_trihedron.Attributes().DatumAspect().LineAspect(Y_AXIS_DIRECTION).SetWidth(2.5)
    a_trihedron.Attributes().DatumAspect().LineAspect(Z_AXIS_DIRECTION).SetWidth(2.5)
