...
def test(gui: MainWindow):
    ...
    test_trsf = gp_Trsf()
    test_trsf.SetTranslation(gp_Vec(0, 0, 10))
    for anB in new_topo_list:
        anB.Move(TopLoc_Location(test_trsf))
        ais_BS = AIS_Shape(anB)
        ais_BS.SetColor(getColor(0, 255, 0))
        gui.display(ais_BS, True)
