...
from pyOCC import COLOR, LocalCONFIG, createTrihedron
...
# end class
class qtViewer3dWidgetManipulator(qtBaseViewerWidget):
    ...
    def __init__(
        ...
        self.zoom_on_center = False

        self.manipulator = AIS_Manipulator()

    # end alternate constructor

    # region selection_color
    @property
    ...
    def selection_color(self, value: tuple[int, int, int]) -> None:
        ...
        # 设置选中后展示的颜色
        self.viewer3d.set_selection_color(1, COLOR(self._selection_color)())
    ...
    def InitDriver(self):
        ...
        self.viewer3d.View.SetBgGradientColors(
            COLOR(self._bg_gradient_color[0])(),
            COLOR(self._bg_gradient_color[1])(),
    ...
    def wheelEvent(self, event):
        ...
        if delta > 0:
            ...
            if modifiers == Qt.KeyboardModifier.ShiftModifier:
                ...
                end_x = start_x + int(zoom_size * 0.1)
                end_y = start_y + int(zoom_size * 0.1)
        else:
            zoom_factor = 0.5
            end_x = start_x - zoom_size
            end_y = start_y - zoom_size
        ...
        if modifiers == Qt.KeyboardModifier.ControlModifier:
            ...
            self.viewer3d.StartZoomAtPoint(start_x, start_y)
            self.viewer3d.ZoomAtPoint(start_x, start_y, end_x, end_y)
        # end if
    ...
    def mousePressEvent(self, event):
        ...
        if self.manipulator.HasActiveMode():
            self.manipulator.StartTransform(
                self.drag_start_position_x, self.drag_start_position_y, self.view
            )
    ...
    def mouseMoveEvent(self, event):
        ...
        if buttons == Qt.MouseButton.LeftButton:
            if self.context.MoreSelected():
                ...
                self.area_select_move(pt.x(), pt.y())
            elif modifiers == Qt.KeyboardModifier.ControlModifier:
                pass
        elif buttons == Qt.MouseButton.RightButton:
            self.zoom_in_window_move(pt.x(), pt.y())
        elif buttons == Qt.MouseButton.MiddleButton:
            if modifiers == Qt.KeyboardModifier.NoModifier:
                self.rotate_move(pt.x(), pt.y())
            elif modifiers == Qt.KeyboardModifier.ControlModifier:
                self.pan_move(pt.x(), pt.y())
        # end if
    ...
    def rotate_move(self, x, y):
        ...
        # ROTATE
        self.cursor = "rotate"
        self.viewer3d.Rotation(x, y)
        self._draw_box = []

    # end def

    def area_select_move(self, x, y):
        # 区域选择
        # SELECT AREA
        # todo 未完成
        self._select_area = True
        self._calculate_draw_box(x, y)
        # self.update()

    # end def
    def pan_move(self, x, y):
        # 平移
        # PAN
        dx = x - self.drag_start_position_x
        dy = y - self.drag_start_position_y
        self.drag_start_position_x = x
        self.drag_start_position_y = y
        self.cursor = "pan"
        self.viewer3d.Pan(dx, -dy)
        self._draw_box = []

    # end def
    def zoom_in_window_move(self, x, y):
        # 局部放大
        # ZOOM WINDOW
    ...
    def set_display_mode(self, mode: str | int):
        if mode == "wireframe" or mode == 0:
            mode = 0
            self.context.SetDisplayMode(mode, True)
        elif mode == "shaded" or mode == 1:
            mode = 1
            self.context.SetDisplayMode(mode, True)
            self.context.DefaultDrawer().SetFaceBoundaryDraw(True)
        elif mode == "solid":
            self.context.SetDisplayMode(1, True)
            drawer = self.context.DefaultDrawer()
            drawer.SetFaceBoundaryDraw(False)

    # end def
    def _calculate_draw_box(self, x, y, tolerance=2):
        """计算框选范围
        ...
        # todo 绘制选择框
        self.drawRubberBand(self.drag_start_position_x, self.drag_start_position_y, x, y)
    ...
    def display_view_cube(self):
        ...
        aDrawer.FaceBoundaryAspect().SetColor(COLOR(228, 144, 255)())
        # 设置立方体标签
        ...
        a_view_cube.SetTextColor(COLOR(241, 217, 37)())
        # 设置立方体面颜色
        a_view_cube.SetBoxColor(COLOR(0, 194, 168)())
        # 设置半透明立方体
...
if __name__ == "__main__":
    app = QApplication([])
    ex = qtViewer3dWidgetManipulator()
    ex.show()
    app.exec()
# end main
